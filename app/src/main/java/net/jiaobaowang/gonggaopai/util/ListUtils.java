...
public class ListUtils {
    ...
    public static <V> boolean isEquals(ArrayList<V> actual, ArrayList<V> expected) {
        ...
        for (int i = 0; i < actual.size(); i++) {
            if (!Validate.isEquals(actual.get(i), expected.get(i))) {
                return false;
            }
        }
        return true;
    ...
    /**
     * 将list转为String，用给定字符分割，如果给定字符为空，则用"，"分割
     ...
     * @param sourceList
     * @param entry
     * @return 如果元素早就存在，则返回false，否则返回true if entry already exist in sourceList, return false, else add it and return true.
     */
    ...
    /**
     * 将List<2>中在List<1>中不存在的元素添加到List<1>中
     * add all distinct entry to list1 from list2
     * 
     * @param <V>
     * @param sourceList
     * @param entryList
     * @return 被加入元素的数量 the count of entries be added
     */
