...
/**
 * PackageUtils
 * 描述：Android包相关工具类，可用于(root)安装应用、(root)卸载应用、判断是否系统应用等
 * 时间：2016年9月14日
 * <ul>
...
public class PackageUtils {
    ...
    public static boolean installNormal(Context context, String filePath) {
        ...
        if (Build.VERSION.SDK_INT >= 24) {
            ...
            Uri apkUri = FileProvider.getUriForFile(context, "com.googosoft.cwyth_html.file_provider", file);
     ...
     * <li>不能在ui线程中启动 Don't call this on the ui thread, it may costs some times.</li>
     ...
     *
     * @param context
     * @param filePath 文件路径 file path of package
     * @param pmParams 安装参数 pm install params
     * @return {@link PackageUtils#INSTALL_SUCCEEDED} means install success, other means failed. details see
     * {@link PackageUtils}.INSTALL_FAILED_*. same to {@link PackageManager}.INSTALL_*
     */
     ...
     * @param packageName  包名  package name of app
     * @param isKeepData 卸载后是否保存app数据和cache文件 whether keep the data and cache directories around after package removal
     * @return <ul>
    ...
    public static Boolean isTopActivity(Context context, String packageName) {
        if (context == null || Validate.isNull(packageName)) {
            return null;
        }
