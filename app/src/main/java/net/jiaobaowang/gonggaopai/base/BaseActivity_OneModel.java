...
import net.jiaobaowang.gonggaopai.util.ToolScanner;
 ...
 */
public abstract class BaseActivity_OneModel extends AppCompatActivity {
    /**
    ...
    private  ToolScanner.OnScanSuccessListener listener;
...
//        hideBottomKey();
    ...
    public boolean dispatchKeyEvent(final KeyEvent event) {
        listener=new ToolScanner.OnScanSuccessListener() {
            @Override
            public void onScanSuccess(String barcode) {
                if(barcode.equals("KEYCODE_ENTER")){
//                    Toast.makeText(cont,"输入的串号为:"+buffer.toString(),Toast.LENGTH_LONG).show();
                    if(Validate.noNull(Const.blandlv)&&Validate.noNull(Const.blandid)){//如果是班级班牌
                        boolean isToLarge=alertDialog(buffer.toString());
                        buffer.setLength(0);
                        if(isToLarge){
                ...
                }else if(barcode.equals("KEYCODE_BACK")){
                    widgetOnKey(event.getKeyCode(),event);
                }else{
                    buffer.append(barcode);
                }
            }
        };
        scanner = new ToolScanner(cont,listener);
    ...
    private boolean alertDialog(String id){
        ...
        String newId=Validate.str2HexStr(id);
